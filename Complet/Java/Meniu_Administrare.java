...
public class Meniu_Administrare extends JPanel {
	...
	public Meniu_Administrare(String user) {
		setSize(790, 630);
		setLayout(null);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					selectStatement.execute(
							"SELECT idutilizator, CNP, Nume, Prenume, Email, IBAN, nr_contract, data_angajarii, salar_neg , nr_ore, nume_utilizator, parola  FROM utilizator WHERE functie != 'SuperAdministrator' AND functie != 'Administrator';");
				...
				try {
					...
					if (linie_edit != -1) {
						...
						selectStatement2.execute("UPDATE utilizator SET idutilizator='" + id + "' ,CNP='" + cnp
								...
								+ em + "' , IBAN='" + ib + "' , nr_contract='" + contr + "' , salar_neg='" + sal
								+ "' ,nr_ore='" + ore + "' ,nume_utilizator='" + u + "' ,parola='" + par
								+ "' WHERE idutilizator='" + id + "';");
					}
					stmt2 = con.createStatement();
					stmt2.execute(
							"SELECT idutilizator, CNP, Nume, Prenume, Email, Numar_Telefon, IBAN, nr_contract, data_angajarii, salar_neg, nr_ore, nume_utilizator, parola  FROM utilizator");
		...
		butonSterge.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					int aux1 = table.getSelectedColumn();
					int idu = (int) table.getValueAt(aux, aux1);
					selectStatement1.execute("DELETE FROM utilizatori WHERE idutilizator='" + idu + "';");
					stmt = con.createStatement();
					stmt.execute(
							"SELECT idutilizator, CNP, Nume, Prenume, Email, IBAN, nr_contract, data_angajarii, salar_neg, nr_ore, nume_utilizator, parola  FROM utilizator");
